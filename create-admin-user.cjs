...
async function createAdminUser() {
  try {
    ...
    if (existingUser.rows.length > 0) {
      ...
      await pool.query(
        'UPDATE users SET password = $1, role = $2, verified = $3 WHERE email = $4',
    ...
    const adminUser = await pool.query('SELECT id, name, email, role, verified FROM users WHERE email = $1', [adminEmail]);
